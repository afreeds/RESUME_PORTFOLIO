  ...
  >
    <div className={`rounded-2xl p-6 md:p-8 h-full backdrop-blur-md transition-all duration-500 ${
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {(project.techTags || project.technologies || []).map((tag) => (
          <span 
            key={tag}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {project.links.demo !== undefined && (
          <a 
            href={project.links?.demo || project.live ||'#'}
            target={(project.links?.demo || project.live) ? "_blank" : undefined}
            rel={(project.links?.demo || project.live) ? "noopener noreferrer" : undefined}
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${
              project.links?.demo || project.live 
                ? 'bg-[#ff2a2a] text-white hover:bg-red-600 hover:shadow-[0_0_20px_rgba(255,42,42,0.4)]' 
                : 'bg-white/5 text-white/40 border border-white/10 cursor-not-allowed'
            }`}
          >
            <ExternalLinkIcon />
            {(project.links?.demo || project.live)? 'Live Demo' : 'Demo Coming Soon'}
          </a>
        )}
